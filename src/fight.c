...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim, bool show_messg)
{
  ...
  { /* 10 and lower */
    if((ch->level - victim->level >= 3) || (victim->level - ch->level >= 3))
    {
      ...
      {
        set_char_color(AT_IMMORT, ch);
        send_to_char("We do not allow murder when there is such a difference in level.\r\n", ch);
      }
      return TRUE;
    }
  }
...
// Only perform if the pet is present in the same room 
  ...
  if(IS_AFFECTED(victim, AFF_SHRINK))
  {
    ...
    if(!IS_NPC(victim))
    {
      if(victim->height != victim->pcdata->tmpheight)
      {
        victim->height = victim->pcdata->tmpheight;
        victim->pcdata->tmpheight = 0;
      }
      if(victim->weight != victim->pcdata->tmpweight)
      {
        victim->weight = victim->pcdata->tmpweight;
        victim->pcdata->tmpweight = 0;
      }
