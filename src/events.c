...
WOLLA(happymoment)
{
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    if(d->connected == CON_PLAYING && (ch = (d->original ? d->original : d->character)) != NULL)
    {
      ...
      {
        send_to_char("&YThe STAFF of &RTimes Lost&Y thank you for your patronage!\r\n\r\n", ch);
        ...
        if(echance(30))
        {
          ...
          send_to_char("&YThe STAFF of &RTimes Lost&Y have granted you a minor boost!&D\r\n", d->character);
        }
        else
        {
          if(ch->hit < ch->max_hit)
          ...
          if(ch->move < ch->max_move)
            ch->move = ch->max_move;
          send_to_char("&YThe STAFF of &RTimes Lost&Y have restored you!&D\r\n", d->character);
        }
      }
    }
  }
