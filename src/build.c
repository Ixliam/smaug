...
const char             *const r_flags[] = {
  ...
  "nosupplicate", "nomagic", "tunnel", "private", "safe", "solitary",
...
// afflictions
  ...
  "hawk_spirit", "wolf_spirit", "enlarge", "insane", "beastmeld", "searing_haze", "r147", "r148", "r149", "r150",
  ...
  if(!IS_SET(ch->pcdata->flags, PCFLAG_BUILDWALK) && !IS_NPC(ch))
  {
    if(!xIS_SET(ch->act, PLR_WIZINVIS))
    {
      if(!VLD_STR(ch->pcdata->bamfout))
        act(AT_IMMORT, "&Y$n vanishes in a swirling mist.", ch, NULL, NULL, TO_ROOM);
      else
...
// bah summoned pets with goto
  ...
  if(!IS_SET(ch->pcdata->flags, PCFLAG_BUILDWALK) && !IS_NPC(ch))
  {
    if(!xIS_SET(ch->act, PLR_WIZINVIS))
    {
      if(!VLD_STR(ch->pcdata->bamfin))
        act(AT_IMMORT, "&Y$n appears in a swirling mist.", ch, NULL, NULL, TO_ROOM);
      else
  ...
  {
    send_to_char("\r\n&YYou vanish into smoke and ash.\r\n\r\n", ch);
  }
...
void do_mset(CHAR_DATA *ch, char *argument)
{
  ...
  switch (ch->substate)
  {
    ...
    case SUB_MOB_DESC:
      if(!ch->dest_buf)
      {
        send_to_char("Fatal error: report to Ixliam.\r\n", ch);
...
//  char *arg4[MIL];
  ...
  {
    send_to_char("\r\nThis room may not be changed without Ixliam's permission.\r\n", ch);
    return;
  }
...
void do_aassign(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You need Ixliam's permission to change anything with tutorials.\r\n", ch);
    return;
  }
...
void do_savearea(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(filename, 256, "%s%s", AREA_DIR, tarea->filename);
