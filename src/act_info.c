...
int new_who(CHAR_DATA *ch, char *argument)
{
  ...
  for(person = first_char; person; person = person->next)
  {
    ...
    if(IS_PLR_FLAG(person, PLR_AFK))
      snprintf(constate, sizeof(constate), " &G[AFK] ");
    else if(IS_PLR_FLAG(person, PLR_RP))
      snprintf(constate, sizeof(constate), " &G[RP] ");
    else
      constate[0] = '\0';
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    if(person)
    {
      ...
      switch (d->connected)
      {
        ...
        case CON_NOTE_TO:
        case CON_EDITING:
          snprintf(constate, sizeof(constate), " &Y[Editing] ");
          break;
      ...
      if(!person->pcdata->rank || !*person->pcdata->rank)
      {
        switch (person->level)
        {
          default:
            ...
            break;
          case MAX_LEVEL:
            mudstrlcpy(rank, "&rADMIN", MIL);
            break;
          case LEVEL_AJ_COLONEL:
            mudstrlcpy(rank, "&rCODER", MIL);
            break;
          case LEVEL_AJ_MAJOR:
            mudstrlcpy(rank, "&rMUD COORDINATOR", MIL);
            break;
          case LEVEL_AJ_CAPTAIN:
            mudstrlcpy(rank, "&rHEAD COUNCIL", MIL);
            break;
          case LEVEL_AJ_LT:
            mudstrlcpy(rank, "&rCO-COUNCIL", MIL);
            break;
          case LEVEL_AJ_SGT:
            mudstrlcpy(rank, "&rCOUNCIL", MIL);
            break;
          case LEVEL_AJ_CPL:
            mudstrlcpy(rank, "&rLIASONL", MIL);
            break;
          case LEVEL_IMMORTAL:
            mudstrlcpy(rank, "&rBUILDER", MIL);
            break;
        }
...
void do_who(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_BLIND(ch))
  {
    pager_printf(ch, "%s [Discord: %s discord.gg/cYB6R8 %s]  [Current Players: %s%s%s]  [%s%d %sMax Since Reboot]&w\r\n", s5, s6, s5, s6, num_punct(pcount), s5, s6, sysdata.maxplayers, s5);
  }
}
...
void do_wizlist(CHAR_DATA *ch, char *argument)
{
  ...
  send_to_char("Here at Times Lost we use the stafflist command.\r\n", ch);
  return;
}
...
void do_config(CHAR_DATA *ch, char *argument)
{
  ...
  if(!VLD_STR(arg))
  {
    ...
    if(IS_BLIND(ch))
    {
      ...
      ch_printf(ch, "%-16s%s\r\n", xIS_SET(ch->act, PLR_AUTOMAP) ? "&Gautomap on" : "&Rautomap off", s2);
