...
void do_bscore(CHAR_DATA *ch, char *argument)
{
  ...
  ch_printf(ch, "Your current  is %+d.\r\n", ch->alignment);
...
/* Volk - fun stuff, display percentage of areas discovered */
  ...
  {
    send_to_pager(divider, ch);
    pager_printf(ch, "&rTimes Lost&C STAFF DATA: Wizinvis [%c]  Wizlevel (%d)\r\n", xIS_SET(ch->act, PLR_WIZINVIS) ? '*' : ' ', ch->pcdata->wizinvis);
    pager_printf(ch, "&CBamfin       : %s\r\n",
                 VLD_STR(ch->pcdata->bamfin) ? ch->pcdata->bamfin :
                 "&C[OoC]: $n please &Rvote&C for us at &Yhttps://www.reddit.com/r/wotlmud/wiki/vote &Cthis message suddenly &REXPLODES!&c and $n is standing here!");
    pager_printf(ch, "&CBamfout      : %s&C\r\n",
                 VLD_STR(ch->pcdata->bamfout) ? ch->pcdata->bamfout : "&C[OoC]: $n please &Rvote&C for us at &Yhttps://www.reddit.com/r/wotlmud/wiki/vote &Cthis message suddenly &REXPLODES!&c and $n is GONE!");
    /*
...
void do_title(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_SET(ch->pcdata->flags, PCFLAG_NOTITLE))
  {
    ...
    send_to_char("The Times Lost STAFF prohibit you from changing your title.\r\n", ch);
    return;
  }
  if(argument[0] == '\0')
  {
...
void do_petratings(CHAR_DATA *ch, char *argument)
{
  ...
  ch_printf(ch, "     &cThe Top &C20&c Players of Times Lost!\r\r\n\n");
