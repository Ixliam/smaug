...
void boot_db(bool fCopyOver)
{
  ...
  log_string("Making Times Lost stafflist");
  make_stafflist();
  ...
  for(;;)
  {
    ...
    if(area_version >= 9)
    {
       ...
       */
      	...
      	{	
        	log_printf("%s: Room (%d) has owner set to (%s), but invalid pfile. CLEARING ROOM.", __FUNCTION__, vnum, pRoomIndex->owner);
  		if(VLD_STR(pRoomIndex->description))
    			STRFREE(pRoomIndex->description);
  		if(VLD_STR(pRoomIndex->name))
    			STRFREE(pRoomIndex->name);
  		if(VLD_STR(pRoomIndex->owner))
...
#ifdef DEBUG
  log_string("Adding to Times Lost stafflist...");
#endif
    ...
    if(staff->level < ilevel)
    {
      ...
      switch (ilevel)
      {
        ...
        case MAX_LEVEL - 8:
          ...
          break;
        case MAX_LEVEL - 9:
          tostafffile("      &rTimes Lost Guest&P");
          break;
        case MAX_LEVEL - 10:
          tostafffile("      &CTimes Lost Retired&c");
          break;
        default:
