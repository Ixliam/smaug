  ...
  if(ms > 15) /* range 1 to 20 -- feel free to * add more */
    switch (number_range(UMAX(1, (ms / 5 - 15)), (ms + 4) / 5))
    {
      ...
      case 19:
        ...
        break;
      case 20:
        t = "A General has no need for that.\r\n";
        break;
    }
  else
  {
...
void clean_obj_queue()
{
  ...
  while(extracted_obj_queue)
  {
    ...
    if(VLD_STR(obj->owner))
      STRFREE(obj->owner);  /* Maybe this should be dispose? Log showed STRFREE called on 
                             * str_dup pointer: handler.c, line 3872 */
    if(VLD_STR(obj->whichcity))
