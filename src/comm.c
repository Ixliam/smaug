...
void                    free_deities( void ); /* OCT 2017 */
void                    freeIdeas(void);
void                    freeBugs(void);
void                    freeTypos(void);
 ...
 * 6D Default Routine
 */
void read_from_oldbuffer(DESCRIPTOR_DATA *d)
{
  int                     i, j, k;
  char                   *p;
  ...
  for(p = d->inbuf; *p; p++)
  {
    ...
    {
      if(memcmp((char *)p, (char *)do_mxp_str, strlen((char *)do_mxp_str)) == 0)
      {
        turn_on_mxp(d);
        /*
...
void write_to_buffer(DESCRIPTOR_DATA *d, const char *txt, unsigned int length)
{
  ...
  while(d->outtop + length >= d->outsize)
  {
    if(d->outsize > 32000)
    {
      ...
      bug("Buffer overflow: (%s). Clearing buffer", d->character ? d->character->name : "???");
//      close_socket(d, TRUE);
      return;
    }
    d->outsize *= 2;
    ...
    switch (*(prompt - 1))
    {
      ...
      case '%':
        *pbuf = '\0';
        ...
        if(stat != 0x80000000)
          snprintf( pbuf, MSL - strlen (buf), "%d", stat );
        pbuf += strlen(pbuf);
        break;
    }
  }
