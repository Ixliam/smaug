...
*                                                                         *
*  Adapted for SMAUG by Ixliam of Times Lost/WOTL                         *
*  Based on original code by Zach of Maelstrom                            *
*                                                                         *
...
void gossip_broadcast(const char * channel, const char * name, const char * game, const char * message) {
  ...
  if ( !strcmp("gossip", channel) ) {
    sprintf( buf, "&R[&Ygossip &WGV&R] &C%s@%s: &c%s", capitalize(name), game, message );
  } else {
    return;
  }
  ...
  for ( d = first_descriptor; d; d = d->next ) {
    ...
    if ( d->connected == CON_PLAYING  ) {    // && !xIS_SET( ch->deaf, CHANNEL_GOSSIP_GV )
