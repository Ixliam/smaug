...
void do_dnd(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch) && ch->pcdata)
    if(IS_SET(ch->pcdata->flags, PCFLAG_DND))
    {
      ...
      {
        interpret(ch, (char *)"title &W : &rFire & Blood&w &W[&RTargaryen&W]");
      }
      else
      {
        interpret(ch, (char *)"title &C No longer DND.");
      }
...
void do_mstat(CHAR_DATA *ch, char *argument)
{
  ...
  if(get_trust(ch) >= LEVEL_AJ_CPL && !IS_NPC(victim) && victim->desc)
    ...
    if(!str_cmp(victim->name, "Delandria"))
    {
      pager_printf(ch,
                   "&cHost: &w127.0.0.1   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\r\n",
                   victim->desc->descriptor, victim->trust, VLD_STR(victim->pcdata->authed_by) ? victim->pcdata->authed_by : "(unknown)");
...
void do_users(CHAR_DATA *ch, char *argument)
{
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    if((d->original && !str_cmp(d->original->name, "Ixliam")) || (d->character && !str_cmp(d->character->name, "Ixliam")))
      snprintf(ip, sizeof(ip), "%s", "127.0.0.1");
    else
    if((d->original && !str_cmp(d->original->name, "Delandria")) || (d->character && !str_cmp(d->character->name, "Delandria")))
      snprintf(ip, sizeof(ip), "%s", "127.0.0.1");
    else
...
void do_cset(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg, "happy"))
  {
    ...
    if(sysdata.happy)
    {
      echo_to_all(AT_RED, "[&WEvent&R] &CIt is time for HAPPY HOUR! Get killing!\r\n", ECHOTAR_ALL);
      ...
      echo_to_all(AT_RED, "[&WEvent&R] &CBoo! the Times Lost Happy Hour is now over.\r\n", ECHOTAR_ALL);
...
void do_saveall(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL * 2, "&RYou have been saved by your friendly &rTimes Lost&R Staff member: %s ...&D", ch->name);
  do_echo(ch, buf);
...
void do_promote(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg2, "level"))
  {
    ...
    if(victim->level < 100)
    {
      snprintf(buf, MIL, "\r\n\r\nPlease congratulate our newest &rTimes Lost &WStaff member %s!", victim->name);
    ...
    interpret(victim, (char *)"title the New &rTimes Lost&D Staff Member.");
    interpret(victim, (char *)"save");
#ifdef I3
    ...
    send_to_char("\r\n&RYou now have a &YTimes Lost - How to be a efficient STAFF member book&R in your hands.\r\n", victim);
    return;
  }
  return;
}
...
void do_reward(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg2, "bonus") && ch->level > 107)
  {
    ...
    send_to_char("\r\n&YYou have been rewarded for your hard work on Times Lost with a extra experience hour.. Thank You!\r\n", victim);
    return;
  }
  victim->pcdata->getsdoubleexp = TRUE;
  victim->pcdata->last_dexpupdate = current_time;
  send_to_char("\r\n&CYou reward them for their hard work.\r\n", ch);
  send_to_char("\r\n&YYou have been rewarded for your hard work on Times Lost with a double experience hour.. Thank You!\r\n", victim);
}
